...
if ( ! empty( $submitted_schedule ) && count( $submitted_schedule['errors'] ) == 0 ) {
	...
	if ( 'add' == $mode ) { // ADD SCHEDULE.
		...
		pb_backupbuddy::log( 'details', 'Attempting to schedule with the following parameters: ' . print_r( $first_run, true ) . print_r( $submitted_schedule['data']['interval'], true ). print_r( $_GET['edit'], true ) );
